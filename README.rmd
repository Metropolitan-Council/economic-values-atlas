...
First, set the following parameters which indicate the state/s and county/ies from which tract data comes from. Right now, this code is set to handle up to 2 states although this can easily be expanded if there is a need. Also set the metro name, and the format of the data inputs. 
...
```{r dlgeos_processtracts, include = F}
...
if(metro == "pdx") {
  ...
  source("./R/data-raw/input_tract_data.R")
}
...
The following code will launch your region's app (!!). To deploy it on an R server, you can click the blue button on the top right hand side of the app that will launch locally.
